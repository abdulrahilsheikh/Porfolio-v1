...
function Loader() {
	...
	return (
		<Html center>
			<div className="text-yellow-100">{progress} % loaded</div>
			<div className="flex rounded-full w-44 bg-slate-800 h-2">
				<div
					style={{ width: `${progress.toFixed(2)}%` }}
					className="bg-yellow-100 rounded-full"></div>
			</div>
		</Html>
	);
}
...
const Model = () => {
	...
	useFrame((data) => {
		...
		data.camera.position.lerp(
			vect.set(-mouse.x * 5, -mouse.y * 5, 0),
			0.05
		);
