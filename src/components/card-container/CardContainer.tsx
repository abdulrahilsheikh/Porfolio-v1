...
function CardContainer({ children, limit = 5, clickHandler }: any) {
	...
	const mutateList = (idx: number, pos?: string) => {
		...
		console.log(reflist.current, idx);

		setState(reflist.current[0]);
		if (point == -1) {
			reflist.current = [
				...reflist.current.slice(1),
				...reflist.current.slice(0, 1),
			];
		} else {
			reflist.current = [
		...
		reflist.current.forEach((a, idx) => {
			...
			if (idx < limit) {
				...
				ref.current.children[
					a
				].style.transform = `translate3d(${100}vw, ${
					limit * 40 * 1.25
				}px, ${0}px)`;
				// }px, ${limit * -50}px)`;
			}
